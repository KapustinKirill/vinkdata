...
Модуль `json_reader.py` предназначен для чтения и обработки JSON-файлов. Он обеспечивает извлечение данных из JSON, что упрощает последующую обработку данных.

### CSV Reader

Модуль `csv_reader.py` предназначен для чтения и обработки CSV-файлов. Он поддерживает чтение файлов как с локального диска, так и потоков, и может работать с различными разделителями и кодировками.
...
Модуль `db_connectors.py` упрощает работу с базами данных, предоставляя инструменты для установления соединений, На данный момент работает только с PostgreSQL так как нужна только она. Основной функционал с записью больших обьемов даных в БД. Основные методы модуля: `insert_data` - загрузка данных в БД. Запись ведется чанками, их можно настраивать через атрибут chunk при создании экземпляра класса. Значение по умолчанию - 1000. `fetch_data` - чтение данных из БД, результат возвращается в виде JSON - корневой каталог значится как result, его нужно использовать как путь при обработке в DATA PROCESSOR. `create_table` - создание таблицы если ее еще нет. Таблица создается исходя из настройки переданной при вызове метода.
