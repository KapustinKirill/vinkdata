...
Модуль `db_connectors.py` упрощает работу с базами данных, предоставляя инструменты для установления соединений, На данный момент работает только с POSTGRES так как нужна только она. Основной функционал с записью больших обьемов даных в БД. Основные методы модуля: `insert_data` - загрузка данных в БД. Запись видется чанками, их можно настраивать через атрибут chunk при создании экземпляра класса. Значение по умолчанию - 1000. `fetch_data` - чтение данных из БД, результат возвращается в виде JSON - корневой каталог значится как result, его нудно использовать как путь при обработке в DATA PROCESSOR. `create_table` - создание таблицы если ее еще нет. Таблица создается исходя из настройки переданной при вызове метода.
